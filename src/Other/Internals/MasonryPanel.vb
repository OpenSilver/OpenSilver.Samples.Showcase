...
Namespace Global.OpenSilver.Samples.Showcase
    ...
    ''' </summary>
    Public Class MasonryPanel
        Inherits Panel
        Private _wall As Object = Nothing
        ...
        Public Sub New()
            ...
            AddHandler Children.CollectionChanged, AddressOf OnChildrenCollectionChanged
            AddHandler Loaded, AddressOf MasonryPanel_Loaded
        End Sub
        ...
        Private Sub OnChildrenCollectionChanged(ByVal sender As Object, ByVal args As NotifyCollectionChangedEventArgs)
            If args.Action = NotifyCollectionChangedAction.Add Then
                For Each child In args.NewItems.OfType(Of FrameworkElement)()
                    AddHandler child.Loaded, AddressOf OnChildLoaded
                Next
            End If
        End Sub

        Private Sub OnChildLoaded(ByVal sender As Object, ByVal e As RoutedEventArgs)
            UpdateMasonryPanel()
        End Sub

        Private Sub UpdateMasonryPanel()
            If IsLoaded Then
                Dim thisDiv = Interop.GetDiv(Me)
                ...
                _wall = Interop.ExecuteJavaScript("
(function() { //we make this a function so that we can get a return type
    var thisInnerDiv = $0.children[0]; //this is the div that contains all the children.
